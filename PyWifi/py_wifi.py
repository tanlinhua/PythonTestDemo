...
class MY_GUI():
    ...
    def show_scans_wifi_list(self, scans_res):
        for index, wifi_info in enumerate(scans_res):
            ...
            self.wifi_tree.insert("", 'end', values=(index + 1, wifi_info.ssid.encode(
                'raw_unicode_escape', 'strict').decode(), wifi_info.bssid, wifi_info.signal))
    ...
    def readPassWord(self):
        ...
        #print("文件路径：%s\n" %(self.getFilePath))
        # 中文问题需要编码回来
        self.get_wifissid = self.get_wifi_value.get().encode(
            'utf-8').decode('raw_unicode_escape', 'strict')
        ...
        while True:
            try:
                ...
                self.bool1 = self.connect(self.pwdStr, self.get_wifissid.encode(
                    'raw_unicode_escape', 'strict').decode('gb18030'))
                #print("返回值：%s\n" %(self.bool1) )
                if self.bool1:
                    # print("密码正确："+pwdStr
                    # res = "密码:%s 正确 \n"%self.pwdStr;
